...
use Akbardwi\LaravelEnvEditor\Exceptions\FileNotFoundException;
use Akbardwi\LaravelEnvEditor\Exceptions\KeyNotFoundException;
use Akbardwi\LaravelEnvEditor\Exceptions\NoBackupAvailableException;
use Akbardwi\LaravelEnvEditor\Workers\Formatters\Formatter;
use Akbardwi\LaravelEnvEditor\Workers\Parsers\ParserV1;
use Akbardwi\LaravelEnvEditor\Workers\Parsers\ParserV2;
use Akbardwi\LaravelEnvEditor\Workers\Parsers\ParserV3;
use Jackiedo\PathHelper\Path;

/**
 * The LaravelEnvEditor class.
 *
 * @package Akbardwi\LaravelEnvEditor
 *
 * @author Akbar Dwi Syahputra <admin@mail.akbardwi.my.id>
 */
class LaravelEnvEditor
{
    /**
    ...
    /**
     * Create a new LaravelEnvEditor instance.
     *
     * @param Container $app
     * @param Config    $config
     *
    ...
    */
    ...
    {
        $this->autoBackup = $this->config->get('laravel-dotenv-editor.autoBackup', true);
        $this->backupPath = $this->config->get('laravel-dotenv-editor.backupPath');

        if (is_null($this->backupPath)) {
            if (method_exists($this->app, 'storagePath')) {
                $this->backupPath = ($this->app->storagePath() . '/laravel-dotenv-editor/backups');
            } else {
                $this->backupPath = $this->app->basePath() . '/storage/laravel-dotenv-editor/backups';
            }
        }

        $this->backupPath = Path::osStyle(rtrim($this->backupPath, '\\/') . '/');

        if ($this->config->get('laravel-dotenv-editor.alwaysCreateBackupFolder', false)) {
            $this->createBackupFolder();
        }
    }
